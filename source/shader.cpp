...
namespace pxdvk
{
	void Shader::init( VkDevice device, std::string shader_path )
	{
		m_device = device;
		...
		{
			LOG_ERROR(
				std::format( "Cant open {} shader file", shader_path ).c_str()
			);
		}
		...
		VK_CHECK(
			vkCreateShaderModule( m_device, &smci, nullptr, &m_shader_module )
		);
	}
	...
	void Shader::create_spirv_shader( std::string& shader_path, std::string& spirv_path )
	{
		...
		std::string to_spirv_command = std::format( "glslc.exe {} -O -o {}", shader_path, spirv_path );
		...
		{
			LOG_SUCCESS(
				std::format( "{} is created", spirv_path ).c_str()
			);
		}
		else
		{
			...
			LOG_ERROR(
				std::format( "{} cannot created", spirv_path ).c_str()
			);
		}
	}
}
